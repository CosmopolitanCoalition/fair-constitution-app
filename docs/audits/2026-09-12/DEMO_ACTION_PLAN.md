...
| 2A. Institutional rooms | Make civic meetings usable and recognizable. | Core rooms, floor controls, archives and role previews implemented; simulated multi-client rehearsal remains | Institution-specific calls, floor positions, names and reconnect behavior pass checks with separate simulated identities/clients. Physical-device rehearsal is deferred. |
| 2B. Economic actions | Give participants useful work, trade and help activities. | E1 hiring and E2 help implemented; E3 organization actions remain | Missing actions work through their existing authority and consent rules, with readable results and recovery. |
| 3. Civic scenario rehearsal | Prove complete attendee journeys through the settled paths. | After required Phase 2 actions | Independent simulated actors complete and repeat each scenario in isolated fixtures, including refusals and recovery. |
...
| 5. Setup and world generation | Produce the intended demo and player-ready beta worlds efficiently. | Audit complete; implementation follows settled demo workflows | Setup validates selected outcomes honestly; interrupted ingestion, mapping, provisioning and simulation resume without skipped work; resource use is measured and bounded. |
| 6. Simulated demo dress run | Verify the final demo presentation. | Last release check; human rehearsal deferred | The chosen demo sequence runs with independent simulated participants, including recovery. Remote networking and physical-device coverage remain recorded limitations until available. |
...
Operator update, 13 September: **all multi-user tests use simulated participants for now**. Human/device dress rehearsal is not a completion gate for this development pass. Setup Step 5 and Dev presets write persistent world state; reuse their sequences inside isolated scenarios rather than running them against the existing world. See [setup and civic evidence audit](SETUP_AND_SCENARIO_AUDIT.md).
...
| P2 | 1 | Finish treasury and long-history reads. | Seller pending orders and private stipend receipts now paginated; treasury and other histories remain | Treasury entry avoids world supply aggregation; wallet payments/stipends, seller pending orders and institution histories are paged with indexed scope and usable return paths. |
...
| R2 | 2A | Rehearse calls with independent simulated participants. | Local two-identity audio/video and rejoin passed; broader automated coverage remains | Separate generated-media clients verify names, floor changes, audio continuity, disconnect/rejoin and denied access to unrelated private rooms. Human/device checks deferred. |
...
| E2 | 2B | Add usable requests for help and participation. | Implemented and tested with separate simulated requester/helper actors | Private draft/publication, private offers, helper selection, withdrawal/reopening and completion work under exact ownership; all new lists page; public market excludes restricted requests. See [help workflow evidence](HELP_WORKFLOW.md). |
...
| S2 | Before 3 | Finish missing interjurisdictional action doors and histories. | Code audit found border/restoration summaries read-only and lifecycle history capped at 25 | Map existing services to authorized proposal/consent/completion controls; page histories; verify union, disintermediation, border and restoration flows with isolated simulated parties. |
...
| G1 | 5 | Give setup an honest world-readiness check. | Simulation's verifying phase is empty; Step 5 completion lacks run-state check | Bounded acceptance records required artifacts and unresolved scopes; demo and beta outcomes are checked independently. |
| G2 | 5 | Repair simulation resume enumeration. | Insert-count termination and OFFSET can skip later missing cohorts on resume | Stable cursor/scanned-row progress; preexisting first page does not skip subsequent work; reruns add no duplicates. |
| G3 | 5 | Reduce geodata/map/provision/simulation startup cost. | Existing host-derived lanes and timing hooks available; global preparation/polling hotspots identified | Use measured per-stage timings, resumable preparation and cached progress; compare cold start/resume/reuse without altering apportionment or resource limits. |
| D1 | 6 | Rehearse the final demo with simulated participants. | Pending; human rehearsal deferred | Final paths, loading feedback, room connections and recovery work together. Record actual checks and distinguish simulated transport evidence from remote/physical-device evidence. |
...
- B3/P2 still cover wallet transaction history (currently 50) and other long institution records. Private stipend receipts and seller pending orders now have 20-row cursors; seller orders retain exact seller/listing checks and first-page recovery after settlement. Treasury still has legacy aggregate paths; completing the money-report reader does not make those paths bounded automatically.
...
Phase 1's incomplete entries are real remaining work, not a status-update omission. New E1/E2 lists are paginated as part of those features, while older B3/P2 treasury and history paths still need their dedicated browsing pass. R2/D1 now use simulated participants; physical-device rehearsal is deferred under the 13 September instruction. Committee testimony still shows its latest 50 entries; its full history pagination belongs to B3 alongside other long institutional records.
...
| Delivered | E2: create, respond to, withdraw and resolve requests for help | Implemented with private responses, scoped pages and simulated multi-actor fixtures. |
| Next | E3: share issuance entry and organization board authority audit | Existing handlers need the remaining usable entry paths and authority review. |
| Before civic rehearsal | B3/P2: older wallet transactions/institution records and treasury reads | Still unfinished. Stipend receipts and seller pending orders are now paginated; existing money-report and hiring pagination do not complete the other separate paths. |
| After required implementation | S1/R2, then language/accessibility, G1–G3 setup and final D1 simulated dress run | Full simulated journeys remain; human/device rehearsal is deferred. Setup must validate both a populated demo and a player-ready beta. |

For continuation on another account or host, start with [NEXT_SESSION_HANDOFF.md](NEXT_SESSION_HANDOFF.md). It records the environment, operator constraints, completed evidence and remaining build priorities.
