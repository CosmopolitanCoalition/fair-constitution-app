...
## Independent-list navigation follow-up

Review of the shared history pager found two remaining OrgSettings controls that still replaced the current query with a stale directory URL: ownership paging and recipient search. Both now merge only their own query fields into the current Inertia URL. Ownership's first page clears only `share_cursor`; recipient searches replace only `issue`, `recipient_q`, `recipient_type` and `recipient_cursor`. The other financial cursors, current recipient search, URL anchor and remembered selected-person draft remain intact. Busy controls refuse repeated visits; existing errors remain visible and clear on retry. No economic action or controller was changed.

`docker exec fc_vite node --experimental-vm-modules --test tests/js/personSelection.test.mjs`: **8 tests passed**, including the original five selector checks and three new compiled-component regressions. The new checks exercise stale links, interleaved financial/ownership/recipient navigation, first-page recovery, changing the recipient search and type, loading and error callbacks, exact independent prop requests, relative URLs suitable for a remote host, and draft persistence after remount. `git diff --check` for the changed page and test passed. Host Node lacked the container-installed Vue dependency; the checks therefore ran in the existing frontend container. No production build, database access or live form submission occurred.
