...
## 10. WAVE 4 RESOLUTION — the race fix (seating half), BUILT 2026-07-29

The operator's 2026-07-29 ruling answered §9's (i)/(ii) with a THIRD shape that
gets (ii)'s guarantee at (i)'s cost: **Type B is one at-large STV race PER CLUMP
(grouped) or PER CHILD (ungrouped) — never one pooled race.** Each clump/child
elects its OWN `rep_floor` seats at-large from its OWN residents, so equal
representation is structural (each unit gets its seats) and the COUNTING CORE is
UNTOUCHED — every unit race is a standard multi-seat at-large STV race
`VoteCountingService` already runs. §9's dilemma dissolves: (ii)'s guarantee, via
many (i)-style races.

**BUILT (per-clump half) — lane 1, this wave:**
- Schema `2026_07_29_220000`: `election_races.type_b_panel_id` (the clump key, FK
  `legislature_type_b_panels`); relaxed `election_races_one_at_large_per_kind` so
  `type_b` may hold many at-large races per election; new
  `election_races_type_b_unit_unique` keyed `COALESCE(type_b_panel_id,
  jurisdiction_id)`.
- `racePlan()`: an ACTIVE grouping → `['mode'=>'panels', panels]` (one race per
  panel, `seats = panel.seats`); flagged + no active grouping → still blocked;
  unflagged → the INTERIM pooled `at_large` (per-CHILD is the follow-up below).
- `createRaces()`: `panels` mode materialises one at-large race per panel,
  `type_b_panel_id` set, `Σ seats = grouping.seats_total = type_b_seats`.
- `RaceFootprint`: LEFT JOIN `legislature_type_b_panel_jurisdictions` so a
  per-clump race enfranchises the UNION of the panel's constituents; per-child and
  plain at-large unchanged (`COALESCE(ldj, ltbpj, er.jurisdiction_id)`).
- Item ⑤ `2026_07_29_220100`: additive CHECK `type_b_seats_per_child ∈ {NULL,2..5}`
  so `AutoscaleResizeRepair`'s silent SQL clamp can never diverge from the ladder.
- Pins: `TypeBDistrictMapperApplyTest` flips blocked→`panels` + materialises 2
  per-clump races; `ElectionStageTest` sim-handoff updated to the per-clump shape.

**COUNTING VERDICT — lane 3's to give.** Lane 1's finding: the core needs ZERO
change (each per-clump race is a standard at-large STV race; San Marino's live
race is the existence proof). This is INPUT to lane 3, not a verdict on
`VoteCountingService`. Lane 3 confirms + pins + retires the stale L223-226 comment.

**NIUE — desk-held.** The flag does NOT clear until lane 3 confirms per-clump
counting. Sequence: schema + racePlan + RaceFootprint land (DONE) → lane 3
verifies → THEN re-activate Niue's draft grouping + clear the flag.

**PER-CHILD (ungrouped) — the sequenced follow-up.** Every unflagged bicameral
chamber (San Marino etc.) must also go per-child: one at-large race per direct
child, `seats = min(pop, rep_floor)` for a ≤5-pop child else `rep_floor`, skip
pop-0 — Σ reproduces `type_b_seats` exactly. Held as its own step: it rewrites the
pooled racePlan pins (`PerKindRacePlanTest`) and the demo-stage 1141 fixtures
(`ElectionStageTest` L185-208), and changes every bicameral chamber's shape.
Interim: an unflagged Type B keeps its single pooled race so chambers still elect.
