...
**WAVE 2 REVIEW RULINGS (operator, 2026-07-29 morning):**

| # | Ruling |
|---|---|
| A2 | **Tour is a togglable MODE; every page is a valid stop.** The nav "tour" control TOGGLES the mode wherever you are (the current page is a stop); `tour.html`/the `/tour` page is effectively the INDEX and the first stop for a fresh start. Lane 6 implements the toggle semantics. |
| A5 | **THERE IS NO TRAINING WINDOW — `seat_training_window_days` is DEAD.** Refined pre-seating flow: on acquiring a role you get the notice; if training isn't done by seating day, then **at your FIRST ACTION in the role the gate redirects you to the per-screen Learn content + a basic comprehension quiz** — pass, then act. "They can't do their job until they know what it is they are doing." The education IS the app's own per-screen Learn layer — multimodal, multilingual, maximally accessible, free, in-app → a non-barrier, under the pillar of GOVERNING EFFECTIVELY. Anyone may train anytime; the one-time civic stipend + achievements stand. Lane 15 redesigns K2 §5.2 to the act-gated model (and surfaces the seated-but-untrained quorum nuance in the design, not as a fresh question). |
| B1 | **One at-large race — CONFIRMED, never reopened.** Grouped panels are the SEAT-ALLOCATION unit inside the single at-large STV race (`seats = groups × rep_floor`); the long-settled at-large doctrine stands untouched. (The rubric row was an implementer's confirm-before-build, badly framed as a question — desk's fault, not a re-litigation.) |
| B2 | **Uneven remainders: step the group size UP to maintain equality; if no even partition exists, the least remainder is borne against the LOWEST-POPULATION jurisdiction(s).** |
| B3 | **Zero-population = INERT — no government, zero seats.** Miniscule populations: representation in full OR capped at the population, whichever is LOWER — **a jurisdiction can never have more representatives than people; its Type A + Type B COMBINED total floors down to its population.** |
| B4 | **The island method — the WorldPop-mask hull (operator's design, check invited):** take the disconnected jurisdiction's WorldPop mask, draw a smoothed imaginary border around all boundary pieces, run the simple line drawing over that pseudo-contiguous shape (ocean/foreign pixels carry no population and are not in the mask), then CUT AWAY district components outside the real boundary mask. Lane 1 technically validates before building ("feel free to check me on it"). |
| C1 | Genesis at one — confirmed. |
| C3 | **The growth formula comes from a REAL-WORLD INVESTIGATION** (services-per-population senses at real scales → a formula for auto-add). Initial setup uses geodata potential (the demo-at-scale posture). Real worlds grow granularly through stages toward peak potential — **and once a playerbase ASSERTS GOVERNANCE (they play, people get elected), institution/room counts become MANUAL, player-governed** (their own courthouses, live rooms, etc.). The investigation is a Wave 3 desk/lane-3 task. |
| C4 | **No extra labeling on premade demo institutions** — the Demo tools' presence IS the mode signal. Question withdrawn as answered. |
| D1 | **Daily batches, BOTH rounds — and the full electoral-season doctrine, recorded verbatim:** seats assign the moment an election concludes; next-cycle signup opens IMMEDIATELY and candidates list immediately; individuals and org-agents may ENDORSE anyone, anytime, for any reason — endorsements are PUBLIC and appear on both the endorsed and endorser's profiles, letting people filter their approval ballots by their network; **the approval vote is the PRIVATE mirror of the endorsement** (the same binary: recommend / not); the most-approved rise; a set number of days before cycle end, the most-approved advance to the STV ranked round with sufficient options for the seats; everyone can still WRITE IN anyone who didn't make the cut; **results from the approval AND STV rounds are BATCHED DAILY** — full-season effort, get-out-the-vote by design. → RankedBallot spec cadence = daily batch; desk audits the built ElectionLifecycle against this doctrine for divergences. |
| A1 | **RULED: PUBLIC.** "It's public if it's government." The Template has no closed-session provision (desk-verified: mandates public trials, public records, transparent elections; the ethics office carries no publicity clause), so the operator's principle governs: **the live oversight console opens to public read** — the same gallery pattern as sessions (lane 3 verified the fold null-safe; a small work item on its next wake). |
| A3 | **RULED: the 2026-07-05 FORK-FIRST ruling stands** — "join or start, THEN account." The MOCKUP was the error; the desk swapped `system/setup.html` to fork-first (steps, indices, assembly, prose) same-day. On JOIN you authenticate against your mesh account; on FRESH/RESTORE the local account founds the world and becomes its first federation account. |
| A4 | **RULED + EXECUTED: orphans of absorptions are removed.** `CandidateProfile.vue` + its surfaces record deleted (desk, zero references verified, tests green); the pattern generalizes. |
| B5 | **RULED: the tie-break must be MEANINGFUL, key TBD via a probe.** Lane 1's mapper build opens with a tie-break analysis — candidate meaningful keys (population-based, compactness-based, seat-deviation) each with determinism guarantees, presented to the operator with options; an arbitrary final key remains underneath for exact ties. SQL-mirror question rides the same probe. |
| B6 | **RULED, general doctrine: NEVER cross-parent joining at ANY ADM level — each peer branch is independent with respect to its own children.** |
| B7 | **RULED: new geographies take effect NEXT TERM, and geodata changes REQUIRE new district maps to be drawn** (Type A districts and Type B groupings alike); sitting members serve out their terms. |
| C2 | **RULED: keep the third founding posture** (`free` — full institutions, no population binding, the tabletop/small-org adopter). |

**The Type B mapper is now UNGATED** — B1–B4, B6, B7 answered; B5's meaningful-key probe is the
build's own first design step, not a gate. Lane 1's Wave 3 order can issue.
