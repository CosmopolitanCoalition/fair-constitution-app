...
QUESTIONS = [
  ...
  {"id":"horizon-idle-throttle","q":"Horizon holds every engine lane in memory while no Setup run is active. On a 16 GB host under the geodata profile the cap derived to 1066 MB, below the idle footprint, and a worker was killed every 10 to 20 seconds at rest. How do the engine lanes give memory back?","status":"open","lane":"scale",
   "detail":"Read in code 2026-09-18. Five of the six Horizon supervisors use balance=simple: each holds its full derived lane count at idle (HostCapacity::idleFleetMb states it). The Horizon floor in get-started is a fixed 1024 MB from a nine-worker count of 2026-09-02; it does not count the supervisor processes or the provision pool, and the budget scaler can trim below it. Your direction 2026-09-18: the formula derives on every box; the Setup engines (geodata, districting, institutions, simulation) keep their max lanes at Setup time; lanes that are not needed throttle back so the game runs; the prewarm has room to widen. Not verified yet (Docker was down): the lowest minProcesses this Horizon version accepts, and the lane ramp time under balance=auto. Both need one live engine start on box E before the change ships.",
   "options":[{"k":"A","t":"Idle throttle. The engine supervisors move to balance=auto: the floor is the lowest lane count Horizon accepts, the max is the derived lane count, so idle lanes exit and a run ramps back to max from its queued lane jobs. The Horizon floor derives from the supervisor tree (master, supervisors, floor lanes, one heavy job) and survives the scaler. The prewarm takes the idle engine share while no run is in flight. This is your stated direction in full; it changes the lane behaviour of every engine and needs one live lane-count check on box E first."},
    {"k":"B","t":"Floor only. Keep the fixed pools. Derive the Horizon floor from the supervisor tree and restore it after the scaler (the demo box proposal). Idle lanes keep their memory. Smallest change; safe before Krakow."},
    {"k":"C","t":"B now, A after Krakow. The floor fix ships for the demo; the idle throttle and the wider prewarm follow with a live check. [desk rec: the demo is three days out and A changes every engine pool]"}]},
  {"id":"sim-roster-vs-real-population","q":"The simulation mints at least 12 people in every place (or the people its elections need, if larger), and it never compares that number with the place's real population. What is the ceiling?","status":"open","lane":"sim",
   "detail":"Read in code 2026-09-19. IdentityStage::run sizes a place at max(rosterSize, the dial sample, the election top-up); rosterSize is max(12, the sum of seats + 1 over the place's own races). The dial sample DOES read the real population (ceil(population x dial / 100), at most 5,000 a leaf). The minimum does not: the result is the LARGER of the two, and nothing caps it at the real population. This base minimum applies with the override checkbox on OR off (the checkbox governs only the election-time top-up for a parent whose bound-up roster is short). Measured on box E, the same planet dataset as the demo (903,500 leaves): 17,233 leaves have zero population and 14,924 have 1 to 11 people, and each of them gets at least 12 simulated residents. At the 0.1 % dial the sample alone is 8,727,621 people; with the 12-person minimum it is at least 16,911,285, because 831,749 leaves (92 %) have a sample under 12. Measured row sizes on box E (2.7M simulated people): about 2.6 to 2.8 KB a person across users, residency rows and the wallet, so the identities phase is about 46 GB at 0.1 %, against 85 GB free on the demo box, before the election, seating and governance rows and the WAL. The only ceiling in the code is the 5,000-a-place sanity cap (the largest lawful roster is Earth, about 3,423). Separate gap: a sim item that files to review has no requeue control (no button, no command), and the run's floor option is fixed when the run is created.",
   "options":[{"k":"A","t":"Keep it. Every place gets the people its elections need, whatever its real population. The demo shows a full set of governments; the count is about 17M people at 0.1 %. [desk rec for Krakow: no engine change three days out; the disk fits with about half the margin the 8.7M figure implied]"},
    {"k":"B","t":"Ceiling at the real population. A place never mints more simulated people than it has residents. A place with fewer residents than its election needs cannot be contested: it files to review (or closes as a lawful inactive place), and the page shows the count."},
    {"k":"C","t":"B, plus zero is zero. A place with zero population mints nobody and closes done with no election, as the districting law already treats a zero-population head. Places with 1 or more residents keep the minimum."},
    {"k":"D","t":"Lower the minimum. Drop the 12-person visible sample to the election need only (seats + 1 per race). Fewer people in small places; the real population is still not consulted."}]},
  ...
  {"id":"boardroom-page-access","q":"Accessibility sweep (W-0447): /rooms/board/{board} returns 403 to a resident who is not a seated member of that election board. Keep the boardroom call private, or render the page read-only for every resident?","status":"open","lane":"a11y",
   ...
   "options":[{"k":"A","t":"Keep the boardroom private to its seated members; record the 403 as an accepted exception in the roster and the report. [desk rec: a deliberating election board is a closed meeting, unlike a chamber or a court]"},
    {"k":"B","t":"Render the room page read-only for any signed-in resident (call transcript or a no-access notice inside the page), join and floor actions still gated; then sweep it."}]},
...
_ANS = {
 ...
 'sim-roster-vs-real-population':('C','Ceiling at the real population, and zero is zero. A place never holds more simulated people than it has real residents. A place with zero population mints nobody and closes done with no election. A place with fewer residents than its election needs cannot be contested: it closes as a counted, visible result, not an error. Option C as the desk wrote it contradicted itself for places with 1 or more residents; the operator settled it the same day: ceiling at real population. Built 2026-09-19 (W-0458).','"C - B, plus zero is zero." Clarification, same day, to the desk question on places with fewer residents than the election needs: "Ceiling at real population".'),
 'horizon-idle-throttle':('C','Floor fix now: the Horizon floor derives from the supervisor tree and survives the scaler. The idle throttle and the wider prewarm follow after Krakow with a live check.','"thats fine. However keep this in the back of your head because if we run into issues scaling the Demo at any step then this may come into play anyway. Example: geodata ingestion us es a different stack then the rest."'),
