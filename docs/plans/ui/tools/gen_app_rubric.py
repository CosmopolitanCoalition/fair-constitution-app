...
QUESTIONS = [
 {"id":"pool-same-bins-band-vs-exception","q":"Same-bin pool landings: an exact in-band vector with a worse deviation band, or an exact vector with a floor exception and a better band?","status":"open","lane":"map",
  "detail":"From the 2026-09-02 pool-landing build (Class A). The landing function now lands every partition to the budget before the band test. For the SAME two bins two exact vectors can exist: one with every district at or above the floor of 5, one with a district below the floor filed as a forced floor exception. The code keeps scoreRank's standing order (worst-district deviation band first, then the number of exceptions), so Milunga [5.55, 4.45] lands 5+5 (both in band 1, no exception) but Saigang [5.89, 4.11] lands 6+4 (2.75%, band 0, one exception) instead of 5+5 (17.8%, band 2, no exception). Cordillera [9.27, 0.73] keeps 9+1 lifted to 2 rather than 5+5 at 585% deviation, which is the settled 2026-08-28 outcome. Two settled rules meet here: the 10% acceptability band and the constitutional floor of 5 as a hard constraint with exceptions only when forced. The question is where the line sits when both vectors are exact.","options":[
   {"k":"A","t":"Keep the standing order: deviation band before exception count. An exception with a clearly better worst district beats an in-band vector with a worse one (Saigang 6+4; Cordillera 9+2). [desk rec: matches the Cordillera ruling]"},
   {"k":"B","t":"Floor first: when an exact all-in-band vector exists it always wins, whatever the deviation (Saigang 5+5; Cordillera would become 5+5 at 585%, so B needs a deviation ceiling to stay sane)."},
   {"k":"C","t":"Floor first within a tolerance: the in-band vector wins when its worst district stays within a bounded band (for example band 2, 20%); beyond that the exception wins (Saigang 5+5; Cordillera 9+2)."}]},
