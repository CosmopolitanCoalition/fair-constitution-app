...
QUESTIONS = [
 ...
 {"id":"cloud-burst-architecture","q":"Cloud world build: what machine shape runs the big ingest, and what happens to it after?","status":"open","lane":"2",
  "detail":"Research 2026-08-29 (docs/plans/launch/CLOUD_BURST_SCALING.md, prices verified via the Azure retail prices API). The engines already size themselves from the host (worker lanes and ETL width derive from cores, memory, and the postgres connection budget), so renting a bigger machine IS the scale-up; no cloud autoscaler is needed for one node. The postgres connection budget caps lanes at 56 planet-wide, and 56 lanes need about 48 cores, so ONE 48-core VM reaches the current ceiling; more boxes add nothing until postgres itself scales. Serverless workers (Container Apps Jobs, Cloud Run) cannot serve the game (no inbound UDP for voice) and hit the same postgres ceiling; they remain a later option for worker bursts only. Verified prices, East US 2 Linux: D64as_v5 (64 cores) $2.75/h pay-go, $0.64/h spot; D32as_v5 $1.38/h, $0.32/h spot. Full build (download + ingest + planet mapping, bounded 16 to 72 h) costs $33 to $200 pay-go or $8 to $46 spot. A deallocated VM bills zero compute; resize works in place and keeps all disks. A spot VM can NEVER convert to a standard VM (creation-time flag), so spot serving requires a second machine on the same data disk.","options":[
 ...
 {"id":"cloud-idle-posture","q":"Cloud box between play sessions, before public launch: keep it running or deallocate?","status":"open","lane":"2",
  "detail":"After the build the box resizes down (see cloud-burst-architecture). A deallocated VM bills zero compute; only the disk (about $38/mo for 256 GB Premium SSD) and the static IP (about $4/mo) keep billing. Starting it for a session takes about 2 minutes and bills $0.17/h on D4as_v5. An always-on D4as_v5 with disk, IP, DNS, and blob comes to about $170/mo (the July Small tier). Once real players use the box it must stay on; this question is only about the window before that.","options":[
 ...
 {"id":"cloud-region","q":"Which Azure region hosts the first cloud box?","status":"open","lane":"2",
  "detail":"Prices in this round were verified for East US 2. Regional variation is real for both pay-go and spot, and spot eviction rates differ per region and size (the portal shows both side by side at create time). Latency to the home mesh favors a US region. A European region would sit closer to September travel but farther from the home boxes. The July plan also noted the host jurisdiction is arguably a constitutional question for a governance project; that consideration is recorded, not resolved.","options":[
