...
| D-21 | **Operator rulings R1–R4 (2026-07-25, given in lane 13's chat — fleet-binding)** | recorded by Lane 7 | ✅ logged 07-25 | **R1 ⚑ CHARTER REVERSAL: economic records SYNC between nodes; privacy is READER privacy on the ballot pattern** — pseudonymous account IDs sync in plaintext so any node verifies sums; the account↔person binding is the single restricted object; guarantee is "no OTHER node learns it", never "nobody can". Roadmap §Phase M's "private-local, never federated" rail + its `FORBIDDEN_SUBJECT_TYPES` additions are **SUPERSEDED and must NOT be built** (roadmap annotated). **R2**: stipend funding is a FOUNDER DIAL (mint or draw from treasury; both engines ship; insufficient treasury = pro-rata short-pay **published as such**, never a silent skip). **R3**: full scope — all 14 economy screens, nine slices L-1→M-3. **R4**: design now, don't walk it (walkthrough cancelled). Also settled in-lane: D-08 asset registration **folds in** (not retired); D-20 #1 GrantService wiring lands in slice L-4 |
| D-22 | **Operator ruling R5 (2026-07-25, lane 3's chat) — EAGER PROVISIONING, "we do it all"** | recorded by Lane 7 | ✅ logged 07-25 | Settles the eager-vs-lazy question §5.6 reserved to the operator. **Eager**: every jurisdiction gets its executive, court, election board and both civic spaces as **real rows** — nothing is a stub awaiting demand. **One refinement, stated once and not re-litigated**: the **Matrix room** fronting a space is materialised **on first contact**, because a single Synapse cannot hold 2.87M rooms — *every place is fully built; only the chat transport is lazy*. Consumers: **lane 2** (host sizing now has its provisioning load), **lane 4** (materialisation cost model in tier terms), **lane 15** (reach-gauge denominators). Built in `fc5fe8f` with `ActivationTierService` + `InstitutionProvisionService` + `ActivationTierTest` |
