...
| **I** | Activation tiers & reach/legitimacy metric | ✅ **ENGINE BUILT 2026-07-25** (lane 3) · UI pending | **Built half:** the activation boot-gate is LIVE — `jurisdiction_activations` state machine, CLK-06 every-minute sweep, settings-cascade threshold (dev default 1), full WF-JUR-01 bootstrap, `jurisdiction:activate`, WI-9 UI status line; CLK-06 already tagged `per_jurisdiction_tier`; the Reach nav surface pre-registered. **BOTH HALVES NOW BUILT (2026-07-25, lane 3):** the tier CURVE shipped as a FOUNDING world property (, real-bound vs free — operator ruling, not an amendable setting), and the reach layer shipped () —  +  +  + real k-anonymity (NOT per-cell), with CI-6 correctly split across BOTH axes: +LeaderProbe elects one tick-leader (the HA concern) while  per jurisdiction is the actual CI-6 rail. **Remaining: the Reach PAGE** —  still carries  against the  contract. **Provides Phase O's denominator.** (Sizing-law note: `cubeRootSeats` is legislature SIZING, a cousin — and its leaf ceiling-9 clamp was retired 2026-07-19, floor-clamp only) |
