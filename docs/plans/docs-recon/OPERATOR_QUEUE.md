...
| B2 | **The locale roster excludes major languages.** The 112-locale base derives from a geodata map of *official* languages per country, and the ladder only subtracts — so **Telugu, Marathi, Punjabi, Gujarati, Kannada, Malayalam, Hausa, Yoruba, Igbo, Javanese** can never enter, despite NLLB support and hundreds of millions of speakers each. | For a cosmopolitan template whose Art. I rights do not key off officialdom, the base list may deserve an explicit widening pass. |
