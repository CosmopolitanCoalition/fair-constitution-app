...
class SimRepairControl
{
    ...
    public function report(SimRun $run, ?string $after = null): array
    {
        ...
        return ['run_id' => $run->id, 'status' => $run->status, 'phase' => $run->phase,
            ...
            'plan_complete' => (bool) ($run->options['repair_plan_complete'] ?? false),
            'authorized' => (bool) ($run->options['repair_apply_authorized'] ?? false),
            'summary' => $run->phase_timings['repair_planning']['summary'] ?? null,
    ...
    public function summarize(SimRun $run): bool
    {
        while (true) {
            $run->refresh();
            if ($run->haltRequested()) { return false; }
