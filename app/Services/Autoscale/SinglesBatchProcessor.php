...
class SinglesBatchProcessor
{
    ...
    {
        $itemFilter = "h.kind = 'single' AND h.map_status = 'running' AND h.claim_token = ?";
        $params     = [$token];

        // 1. Activate the STAMPED container (world-build fact, operator plan
        //    2026-08-31): the founding context activates without a board.
        //    Legislatures that already have an active map (operator work)
        //    are left untouched — the newest-active lateral below adopts it.
        DB::statement("
            UPDATE legislature_district_maps m
               SET status = 'active', effective_start = CURRENT_DATE, updated_at = now()
              FROM apportionment_ledger h
             WHERE {$itemFilter}
               AND m.id = h.map_id
               AND m.status = 'draft'
               AND m.deleted_at IS NULL
               AND NOT EXISTS (SELECT 1 FROM legislature_district_maps m2
                                WHERE m2.legislature_id = m.legislature_id
        ...
        DB::statement("
            UPDATE apportionment_ledger h
               SET map_status = 'done',
                   ...
                   drift = s.seated - COALESCE(h.head_seats, 0),
                   started_at = COALESCE(h.started_at, now()),
                   finished_at = now(),
                   updated_at = now()
              FROM (
                    SELECT h2.legislature_id, COALESCE(SUM(d.seats), 0) AS seated
                      FROM apportionment_ledger h2
                      JOIN legislatures l ON l.id = h2.legislature_id AND l.deleted_at IS NULL
                     ...
                     WHERE h2.kind = 'single'
                       AND h2.map_status = 'running' AND h2.claim_token = ?
        ...
        $doneCount = (int) DB::table('apportionment_ledger')
            ->where('kind', 'single')
            ->where('claim_token', $token)->where('map_status', 'done')->count();
