...
class BoardGovernorNomination implements FormHandler
{
    ...
    public function handle(?User $actor, array $payload): array
    {
        ...
        if (($departmentId === null) === ($organizationId === null)) {
            throw new ConstitutionalViolation(__('Select one department or Common Good Corporation for this nomination.'), 'Art. III §4/§5');
        }
        ...
        $owner = is_string($id) && Str::isUuid($id)
            ...
            : null;
        if ($owner === null) {
            throw new ConstitutionalViolation(__('Select an existing department or Common Good Corporation.'), 'Art. III §4/§5');
        }
        if (isset($payload['jurisdiction_id']) && (! is_string($payload['jurisdiction_id']) || $payload['jurisdiction_id'] !== (string) $owner->jurisdiction_id)) {
            throw new ConstitutionalViolation(__('The nomination must use this institution\'s jurisdiction.'), 'Art. III §4/§5');
        }
        ...
        if (! is_string($executiveId) || ! Str::isUuid($executiveId)) {
            throw new ConstitutionalViolation(__('This institution needs an overseeing executive before governors can be nominated.'), 'Art. III §4/§5');
        }
        ...
        if (! is_string($nominee) || ! Str::isUuid($nominee)) {
            throw new ConstitutionalViolation(__('F-EXE-001 names the nominee.'), 'Art. III §4');
        }
        $dossier = $payload['dossier'] ?? null;
        if ($dossier !== null && (! is_string($dossier) || mb_strlen($dossier) > 20000)) {
            throw new ConstitutionalViolation(__('Keep the nomination dossier within 20,000 characters.'), 'CGA Forms Catalog (F-EXE-001)');
        }
