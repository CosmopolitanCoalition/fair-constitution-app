...
final class InstitutionActWorkspace
{
    ...
    public function context(Legislature $legislature, ?User $user): array
    {
        ...
        foreach (self::ACTIONS as $key => $meta) {
            $reason = match ($key) {
                'delegate-executive' => $executive?->status === 'forming' ? null : __('Delegation requires an executive awaiting formation.'),
                'elect-executive' => in_array($executive?->status, ['forming', 'delegated'], true) ? null : __('The executive must be awaiting formation or currently delegated.'),
                'create-department' => in_array($executive?->status, ['delegated', 'elected'], true) ? null : __('Delegate or elect this jurisdiction’s overseeing executive first.'),
                'create-court' => $court?->status === 'forming' ? null : __('Court creation requires a judiciary awaiting formation.'),
                'elect-court' => $court?->status === 'appointed' ? null : __('Conversion requires an appointed court.'),
                default => null,
            };
            $actions[] = ['key' => $key, 'name' => __($meta['name']), 'ready' => $reason === null, 'reason' => $reason];
        }
        ...
        return ['actions' => $actions, 'canFile' => $member !== null, 'canCast' => $member !== null && (string) $legislature->speaker_id !== (string) $member->id,
            ...
            'role' => $member === null ? __('Public preview') : ((string) $legislature->speaker_id === (string) $member->id ? __('Speaker') : __('Legislator')),
    ...
    public function proposals(Request $request, Legislature $legislature): array
    {
        ...
        return ['records' => $page->getCollection()->map(function ($proposal) use ($votes, $names, $member, $legislature) {
            ...
            if (isset($payload['kind'])) {
                $details[] = ['label' => __('Department function'), 'value' => ['chief_executive' => __('Chief executive'), 'treasury' => __('Treasury'), 'defense' => __('Defense'), 'state' => __('State'), 'justice' => __('Justice'), 'other' => __('Other')][$payload['kind']] ?? __('Other')];
            }
            foreach (['delegated_scope' => __('Delegated powers'), 'member_count' => __('Executive seats'), 'target_type' => __('Executive structure'), 'name' => __('Name'), 'court_name' => __('Court name'), 'charter_text' => __('Charter'), 'function_text' => __('Court function'), 'judge_count' => __('Elected judges'), 'judges_per_constituent' => __('Judges per constituent'), 'committee_judge_count' => __('Committee-selected judges'), 'owner_seats' => __('Governor seats'), 'goods_services' => __('Goods and services')] as $key => $label) {
                if (isset($payload[$key]) && is_scalar($payload[$key])) {
                    $details[] = ['label' => $label, 'value' => (string) $payload[$key]];
                }
            }
            if (is_string($payload['charter'] ?? null)) {
                $details[] = ['label' => __('Charter'), 'value' => $payload['charter']];
            } elseif (is_array($payload['charter'] ?? null)) {
                foreach (['function_text' => __('Department function'), 'powers_text' => __('Department powers'), 'reporting_interval_months' => __('Report interval (months)')] as $key => $label) {
            ...
            return ['id' => $proposal->id, 'name' => $payload['name'] ?? $payload['court_name'] ?? __($names[$proposal->proposal_kind]), 'action' => __($names[$proposal->proposal_kind]),
    ...
    public function processes(Request $request, Legislature $legislature): array
    {
        ...
        return ['records' => $page->getCollection()->map(fn ($process) => [
            'id' => $process->id, 'name' => $process->kind === 'judiciary_convert' ? __('Elected court consent') : __('Elected executive consent'),
    ...
    public function constituents(Request $request, Legislature $legislature): ?array
    {
        ...
        return ['name' => $process->kind === 'judiciary_convert' ? __('Court conversion: constituent decisions') : __('Executive conversion: constituent decisions'),
            'records' => $page->getCollection()->map(function ($consent) use ($byId, $byPlace, $places, $id) {
                ...
                return ['id' => $consent->id, 'name' => $places[$consent->jurisdiction_id] ?? __('Constituent jurisdiction'), 'result' => $consent->result,
    ...
    private function vote(?ChamberVote $vote, Legislature $legislature, ?LegislatureMember $member, string $type, ?string $subjectId, bool $pending): ?array
    {
        ...
        return ['tally' => $this->votes->tallyProps($vote), 'my_cast' => $cast?->value,
            ...
            'read_only_reason' => $member === null ? __('Serving members of this legislature cast public votes.') : ($speaker ? __('The Speaker votes only to break a tied decision.') : null)];
    }
