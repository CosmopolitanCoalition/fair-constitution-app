...
final class CgcGovernorWorkspace
{
    ...
    public function context(): ?array
    {
        ...
        $reason = match (true) {
            $org->type !== Organization::TYPE_COMMON_GOOD_CORP || $org->status !== Organization::STATUS_ACTIVE || ! $org->is_active || $org->dissolved_at !== null => 'This common-good corporation is not active.',
            ! $boardCurrent || ! in_array($this->board->status, [Board::STATUS_FORMING, Board::STATUS_ACTIVE], true) => 'This organization has no current governing board available for appointments.',
            $executive === null || ! in_array($executive->status, [Executive::STATUS_DELEGATED, Executive::STATUS_ELECTED, Executive::STATUS_CONVERSION_VOTED], true)
                || (string) $executive->jurisdiction_id !== (string) $org->jurisdiction_id => 'The assigned overseeing executive is unavailable or belongs to another jurisdiction.',
            $legislature === null || ! in_array($legislature->status, ['active', 'forming'], true)
                || (string) $legislature->jurisdiction_id !== (string) $org->jurisdiction_id => 'The creating legislature is unavailable or belongs to another jurisdiction.',
            default => null,
        };
        ...
        return $this->resolved = [
            ...
            'executive_name' => $executive ? ($places[$executive->jurisdiction_id] ?? 'Assigned').' executive' : null,
            'legislature_href' => $legislature ? '/legislatures/'.$legislature->id.'/chamber' : null,
            'legislature_name' => $legislature ? ($places[$legislature->jurisdiction_id] ?? 'Creating').' legislature' : null,
            ...
            'reason' => $reason ?? ($executiveMember === null ? 'Nominations are filed by seated members of this organization’s overseeing executive.'
                : ($vacancies === 0 ? 'No governor seat is vacant. Existing nominations and terms are shown below.' : null)),
    ...
    public function appointments(Request $request): array
    {
        ...
        if (! empty($input['governor_cursor'])) {
            try {
                ...
                throw ValidationException::withMessages(['governor_cursor' => 'This appointment page link is invalid. Return to the first page.']);
            }
        }
        ...
        return ['rows' => array_map(function ($row) use ($ctx, $names, $people, $votes, $casts, $terms) {
            ...
            return ['id' => (string) $row->id, 'seat_no' => (int) $row->seat_no, 'status' => $row->status, 'current' => $current,
                ...
                'dossier' => $record,
                'consent_notice' => $row->consent_vote_id && ! $validVote ? 'The linked consent record is unavailable or does not match this appointment.' : null,
                'consent' => $validVote ? ['tally' => $this->votes->tallyProps($vote), 'cast_url' => '/votes/'.$vote->id.'/cast',
                    ...
                    'read_only_reason' => ! $current ? 'This is an earlier nomination for the seat.' : ($ctx['isSpeaker'] ? 'The Speaker does not cast an ordinary consent vote.' : null),
                ] : null];
