...
class JudiciaryController extends Controller
{
    ...
    public function show(Request $request, Judiciary $judiciary): Response
    {
        ...
        return Inertia::render('Judiciary/Home', [
            ...
            'nominationContext' => fn () => app(\App\Support\JudicialNominationWorkspace::class)->context($judiciary, $request->user()),
            'vacantSeats' => fn () => app(\App\Support\JudicialNominationWorkspace::class)->seats($request, $judiciary),
            'judicialCommittees' => fn () => app(\App\Support\JudicialNominationWorkspace::class)->committees($request, $judiciary),
            'judicialProposals' => fn () => app(\App\Support\JudicialNominationWorkspace::class)->proposals($request, $judiciary),
            'judicialNominees' => fn () => app(\App\Support\GovernorNomineeDirectory::class)->page($request, 'court:'.$judiciary->id, $judiciary->jurisdiction_id, '/judiciaries/'.$judiciary->id),
