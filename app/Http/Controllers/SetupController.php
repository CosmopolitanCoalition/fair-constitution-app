...
class SetupController extends Controller
{
    ...
    /**
     * The .env read/write helpers moved to App\Support\EnvFile (W-0448) so the
     * Step-2 media library controller writes MEDIA_DIR the same way this writes
     * ARCHIVE_PATH. These thin delegates keep every call site here unchanged.
     */
    private function readEnvValue(string $key): ?string
    {
        return EnvFile::read($key);
    }

    private function normalizePath(string $path): string
    {
        return EnvFile::normalizePath($path);
    }
