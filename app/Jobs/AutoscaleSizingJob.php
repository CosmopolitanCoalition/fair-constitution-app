...
class AutoscaleSizingJob implements ShouldQueue
{
    ...
    private function runSizing(AutoscaleRun $run): void
    {
        ...
        for ($lvl = 0; $lvl <= $admMax; $lvl++) {
            ...
            // THE ONE LEAF STATEMENT: the shared owner
            // (AutoscaleEnumeration::seedLeafLegislatures) carries the
            // population reality cap, the pop-0 skip, and the quorum cap
            // for every caller — this pass and the ingest tail alike.
            $leavesSoFar += \App\Support\AutoscaleEnumeration::seedLeafLegislatures($lvl, $floor);
        ...
        // B3 — founding maps, minted CHUNKED (the ETL rule: bounded committed
        // batches, visible + resumable — never one opaque planet-wide
        // statement). Draft status; activation is the finalize step's flip.
        // The map_id stamp on the item is what every scope worker files into
        // — no per-worker ensureFoundingMap, no two-maps race.
        \App\Support\AutoscaleEnumeration::mintFoundingMaps((string) $run->id, function (int $n) use ($run) {
            Log::info('Autoscale sizing: founding maps minted', ['run_id' => $run->id, 'count' => $n]);
            $this->heartbeat($run);
        });
        \App\Support\AutoscaleEnumeration::stampFoundingMapIds((string) $run->id, function (int $n) use ($run) {
            $this->heartbeat($run);
        });
        ...
        \App\Support\AutoscaleEnumeration::mintRootScopes((string) $run->id, function (int $n) use ($run) {
            $this->heartbeat($run);
        });
