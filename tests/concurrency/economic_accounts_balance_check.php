...
if (($argv[1] ?? '') !== '--run') { echo "Opt-in: php tests/concurrency/economic_accounts_balance_check.php --run\n"; exit(0); }
...
try {
    ...
    // The economic_accounts shape the baseline ships (id, kind, currency_id, balance, status).
    DB::statement("CREATE TABLE economic_accounts (
        id uuid PRIMARY KEY, kind varchar(16) NOT NULL DEFAULT 'user', currency_id uuid,
        balance numeric(24,6) NOT NULL DEFAULT 0, status varchar(16) NOT NULL DEFAULT 'open',
        created_at timestamptz, updated_at timestamptz, deleted_at timestamptz)");

    $migration = require dirname(__DIR__, 2).'/database/migrations/2026_09_14_000100_add_balance_floor_to_economic_accounts.php';
    $migration->up(); $migration->up(); // rerunnable — the second call is a no-op

    $present = DB::selectOne("SELECT 1 AS present FROM pg_constraint WHERE conname = 'economic_accounts_balance_check' AND conrelid = 'economic_accounts'::regclass");
    check($present !== null, 'The balance CHECK was not added');

    // A non-negative balance inserts fine.
    DB::table('economic_accounts')->insert(['id' => '60000000-0000-4000-8000-000000000001', 'kind' => 'user', 'balance' => '10.000000']);
    emit(['nonnegative_insert' => true]);

    // Driving the balance negative is rejected with a check-violation (SQLSTATE 23514).
    try {
        DB::table('economic_accounts')->where('id', '60000000-0000-4000-8000-000000000001')->update(['balance' => '-1.000000']);
        throw new RuntimeException('The CHECK accepted a negative balance');
    } catch (Illuminate\Database\QueryException $e) {
        check($e->getCode() === '23514', 'Unexpected failure code: '.$e->getCode());
        emit(['negative_balance_rejected' => true]);
    }

    // A direct negative insert is rejected too.
    try {
        DB::table('economic_accounts')->insert(['id' => '60000000-0000-4000-8000-000000000002', 'kind' => 'user', 'balance' => '-5.000000']);
        throw new RuntimeException('The CHECK accepted a negative insert');
    } catch (Illuminate\Database\QueryException $e) {
        check($e->getCode() === '23514', 'Unexpected failure code on insert: '.$e->getCode());
        emit(['negative_insert_rejected' => true]);
    }

    emit(['rerunnable_migration' => true, 'balance_floor_enforced' => true]);
