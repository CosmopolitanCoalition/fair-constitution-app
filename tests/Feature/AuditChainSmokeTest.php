...
class AuditChainSmokeTest extends TestCase
{
    ...
    public function test_registry_holds_exactly_132_canonical_forms(): void
    {
        ...
        // testimony / carve-out removal) + the Phase K-3 F-SOC-004 (M-5 physical-law
        ...
        // + F-ORG-008 (Organization Market Participation, Design Round 2 build
        // piece 4): the org economy's ACTS door — first action issue_shares,
        // equity on the named ownership plane (Art. III §5), never a currency.
        // + F-IND-020 (Resident Agreement, Design Round 2 build pieces 5+6):
        // the citizen consent-plane door — person-to-person / N-party
        // agreements AND clause-redline actions (Art. I, freedom to contract).
        // + F-IND-021 (Share Trade, Wave 4 ② build): holder-to-holder secondary
        // share RESALE on the exchange (offer/buy/cancel) — an INDIVIDUAL act
        // (R-01), units on the named ownership plane, money on the account plane.
        // + F-CHR-005/006 (Committee Meeting Open / Adjournment, Wave 5 ① build):
        // the durable meeting state machine the keystone hearing runs through —
        // scheduled→open (gavel in) and open→adjourned+minutes. Before these the
        // meeting lifecycle had no scheduled→open→adjourned transition via any
        // form/route/handler; the W4 exit-walk force-wrote those states.
        //
        ...
        // + F-ORG-011 (IO-5, operator ruling 2026-09-13, org-staff-delegation-
        //   model = A): Staff Delegation. The organization agent grants/revokes
