...
return new class extends Migration
{
    ...
    private const REPLACEMENT = 'ledger_public_account_seek_idx';
    private const LOCK = 0x4c454447494458; // LEDGIDX; never the payment append lock
    ...
    private function change(bool $restore): void
    {
        ...
        if ($db->transactionLevel() !== 0 || $pdo->inTransaction()) {
            throw new RuntimeException('Ledger index maintenance requires no open transaction.');
        }
        ...
        try {
            ...
            if (! $locked) {
                throw new RuntimeException('Another ledger index migration owns this database. Retry after it finishes.');
            }

            $table = $this->one($pdo, "SELECT c.oid, c.relkind, c.relpersistence FROM pg_class c
                WHERE c.oid=to_regclass('public.ledger_entries')");
            if (! $table || $table->relkind !== 'r' || $table->relpersistence !== 'p') {
                throw new RuntimeException('Expected the ordinary persistent public.ledger_entries table.');
            }
            if (! $restore) {
                $replacement = $this->index($pdo, self::REPLACEMENT, $table->oid,
                    ['account_type', 'account_id', 'currency_id', 'seq']);
                if (! $replacement || ! $replacement->indisvalid || ! $replacement->indisready || ! $replacement->indislive) {
                    throw new RuntimeException('The replacement ledger account index must exist and be valid, ready and live.');
                }
            }
            if ($this->one($pdo, 'SELECT pid FROM pg_stat_progress_create_index WHERE relid=? LIMIT 1', [$table->oid])) {
                throw new RuntimeException('A ledger index build is in progress. Retry after it finishes.');
            }
            $old = $this->index($pdo, self::OLD, $table->oid, ['account_type', 'account_id']);
            if ($old && $this->one($pdo, "SELECT 1 FROM pg_depend
                ...
                throw new RuntimeException('The old ledger account index has dependencies; refusing to remove it.');
            }
            ...
            if ($restore) {
                $pdo->exec('CREATE INDEX CONCURRENTLY '.self::OLD.' ON public.ledger_entries (account_type, account_id)');
                $rebuilt = $this->index($pdo, self::OLD, $table->oid, ['account_type', 'account_id']);
                if (! $rebuilt?->indisvalid || ! $rebuilt->indisready || ! $rebuilt->indislive) {
                    throw new RuntimeException('The restored ledger account index is not ready. Retry the restoration.');
                }
            }
        } finally {
    ...
    private function index(PDO $pdo, string $name, int $table, array $keys): ?object
    {
        ...
        if ($index && ($index->relkind !== 'i' || $index->amname !== 'btree'
            ...
            || preg_match('/[^0 ]/', (string) $index->indoption)
            || json_decode($index->keys ?? '[]', true) !== $keys)) {
            throw new RuntimeException('Unexpected definition or protected use for ledger index '.$name.'. No change made to that index.');
        }
