...
async function finishSetup() {
    ...
    try {
        ...
        if (!res.ok) {
            error.value = data.error || t('c_setup.step6_confirm.err_finish', { status: res.status })
            return
        }
        finished.value = true
    } catch (e) {
...
<template>
    <div class="max-w-4xl mx-auto px-6 py-8 w-full">
        ...
        <header class="mt-8 mb-6">
            <h1 class="text-3xl font-bold text-white mb-2">{{ t('c_setup.step6_confirm.heading', 'Confirm & Close') }}</h1>
            <p class="text-gray-300 leading-relaxed">
                {{ t('c_setup.step6_confirm.intro', 'Review the shape of the instance, then close setup. From this point the wizard is finished: the constants and the game mode lock, and the world opens.') }}
            </p>
        </header>
        ...
        <section v-if="!finished" class="bg-gray-900 border border-gray-800 rounded-lg p-6 mb-6">
            <h2 class="text-white font-semibold mb-3">{{ t('c_setup.step6_confirm.finish_heading', 'What happens when you click Finish') }}</h2>
            <ul class="text-sm text-gray-300 space-y-2 pl-4 list-disc">
                <li>{{ t('c_setup.step6_confirm.finish_point_1', 'The confirmation and the data-quality snapshot are recorded on the instance settings.') }}</li>
                <li v-html="t('c_setup.step6_confirm.finish_point_2', '<code>setup_completed_at</code> is stamped. <code>/setup</code> redirects home from then on.')"></li>
                <li>{{ t('c_setup.step6_confirm.finish_point_3', 'Nothing else runs. Institutions were Step 4\'s; the simulation is Step 5\'s.') }}</li>
            </ul>
        </section>

        <section v-if="finished" class="bg-emerald-900/20 border border-emerald-700/50 rounded-lg p-8 mb-6 text-center">
            <div class="text-5xl mb-4">🎉</div>
            <h2 class="text-2xl font-bold text-emerald-200 mb-2">{{ t('c_setup.step6_confirm.closed_heading', 'Setup closed') }}</h2>
            <p class="text-emerald-100/80 max-w-xl mx-auto leading-relaxed">
                {{ t('c_setup.step6_confirm.closed_body', 'The instance is set up. Legislatures are sized, districts are drawn, institutions stand. Welcome to your fair constitution.') }}
            </p>
        </section>

        <div v-if="error" class="bg-red-900/30 border border-red-800 rounded p-4 text-sm text-red-200 mb-6">{{ error }}</div>

        <ExportBackupPanel :title="t('c_setup.step6_confirm.export_title', 'Export this instance (backup / sync)')" />

        <div class="flex justify-between pt-4 border-t border-gray-800 mt-4">
            <a v-if="!finished" href="/setup" class="text-gray-400 hover:text-gray-200 text-sm px-2 py-2">{{ t('c_setup.step6_confirm.back', '← Back') }}</a>
            <span v-else></span>
            ...
            >
                {{ t('c_setup.step6_confirm.btn_enter', 'Enter Instance →') }}
            </button>
        </div>
    </div>
</template>
