...
/* Function-ref registry keyed by STABLE candidacy_id (never by index — a
   removal makes the unmounting row's ref(null) fire after the shifted
   row's assignment, nulling the shared index slot and dropping focus).
   Btn is a single-root component, so the instance's $el IS the button. */
const controlEls = new Map(); // candidacy_id → { up, down, remove }
function setControlRef(kind, entry) {
    return (instance) => {
        let rec = controlEls.get(entry.candidacy_id);
        if (!rec) {
            rec = {};
            controlEls.set(entry.candidacy_id, rec);
        }
...
function focusControl(kind, entry) {
    const rec = controlEls.get(entry?.candidacy_id) ?? {};
    const order = {
        up: ['up', 'down', 'remove'],
        down: ['down', 'up', 'remove'],
    ...
    >
        ...
        >
            ...
            <span style="flex: 1; color: var(--gov-fg)">
                {{ entry.name }}
                {{ ' ' }}
                <TagChip v-if="entry.write_in">write-in</TagChip>
            </span>
